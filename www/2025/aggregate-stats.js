...
var teamGraphs={
	...
	"Scoring Element Cycles":{
		graph:"stacked",
		tr:'Puanlama Elemanı Döngüleri',
		pt:'Ciclos de elementos de pontuação',
		fr:'Cycles des éléments de score',
		he:'מחזורי ניקוד של אלמנטים',
		zh_tw:'計分元素週期',
...
var aggregateGraphs={
	...
	"Human Player Accuracy":{
		graph:"bar",
		tr:'İnsan Oyuncu Doğruluğu',
		pt:'Precisão do jogador humano',
		fr:'Précision du joueur humain',
		he:'דיוק שחקן אנושי',
		zh_tw:'人類玩家準確度',
...
function showSubjectiveScouting(el,team){
	promiseSubjectiveScouting().then(subjectiveData => {
		...
		el.append($('<fieldset>').append($('<legend data-i18n=subjective_strengths_question>').append($('<div style=white-space:pre-wrap>').text(dat.strengths||""))))
		el.append($('<fieldset>').append($('<legend data-i18n=subjective_weaknesses_question>').append($('<div style=white-space:pre-wrap>').text(dat.weaknesses||""))))
